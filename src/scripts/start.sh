...
echo "Initilizing..."
if [ -z "$MY_FC_DIR" ]; then
  echo "No MY_FC_DIR environment variable, please make sure it exists!"
  exit 1
fi
cp "$MY_FC_DIR"/conf/wordpress.conf /etc/nginx/sites-enabled/
chmod 777 /mnt/auto/conf/wordpress/wp-content
